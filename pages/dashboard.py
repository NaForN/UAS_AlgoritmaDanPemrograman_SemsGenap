...
if menu:
    df_menu = pd.DataFrame(menu, columns=["Nama", "Harga", "Stok"])
    st.table(df_menu[["Nama", "Stok"]])
else:
...
if transaksi:
    ...
    with col2:
        st.metric("Total Item Terjual", total_item)

    col3, col4 = st.columns(2)
    with col3:
        st.metric("Total Pendapatan", f"Rp {total_pendapatan}")
        st.caption(f"Detail: Rp {total_pendapatan:,}")
    with col4:
        st.metric("Rata-rata per Transaksi", f"Rp {int(rata_transaksi)}")
        st.caption(f"Detail: Rp {rata_transaksi:,.2f}")
